...
const JEBENA_BUNA_ADDON = {
  ...
  description: "4:00 PM ritual roasted coffee beans with frankincense aroma, poured tableside.",
  price: 70,
  image: "https://images.unsplash.com/photo-1447933601403-0c6688de566e?auto=format&fit=crop&w=300&q=70",
};

/**
 * BasketPage — the cart / basket review step. Cart state lives in the shared
 * store (see context/StoreContext.jsx) so it persists across reloads and
 * stays in sync with the navbar's cart pill and the checkout page.
 */
export default function BasketPage() {
  ...
  // BasketLineItem calls back with a dish's `id`, but the store keys cart
  // lines by `key` (id + chosen options) so two configurations of the same
  // dish don't merge — resolve id -> key here rather than touching the
  // (unrelated) line-item component.
  const findKey = (id) => cartItems.find((item) => item.id === id)?.key;
  const handleQuantityChange = (id, quantity) => {
    const key = findKey(id);
    if (key) setQuantity(key, quantity);
  };
  ...
  const ledgerProps = {
    ...
    rows: [
      {
        label: `Items Subtotal (${itemCount} item${itemCount === 1 ? "" : "s"})`,
    ...
    coupon:
      totals.discount > 0 ? { label: `${couponCode.toUpperCase()} applied`, value: `− ETB ${totals.discount}` } : undefined,
    ...
    exploreTo: "/menu",
    trust: [
      { icon: <FiBox />, text: "Piping warm delivery in woven mesob packaging" },
      { icon: <FaTruckFast />, text: "Telebirr, CBE Birr, cash & card on delivery" },
      { icon: <FiLock />, text: "Encrypted checkout & real-time dispatcher SMS" },
    ],
    upsell: {
      icon: <FaMugHot />,
        ...
        <div className="basket-layout">
          <div className="basket-main">
            <div className="basket-group__head">
              <h2>
                {basketGroup.heading} <span>({itemCount} handcrafted selection{itemCount === 1 ? "" : "s"})</span>
              </h2>
              <button type="button" className="link-quiet" onClick={handleClearTable}>
            ...
            <div className="basket-lines">
              {cartItems.length === 0 ? (
                <p className="basket-empty">
                  Your mesob is empty for now — head back to the menu to build your feast.
                </p>
              ) : (
                cartItems.map((item) => (
                  <BasketLineItem
            ...
            <EtiquettePanel {...etiquette} note={cartNote} onNoteChange={setCartNote} />
          </div>
